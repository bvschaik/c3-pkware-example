...
public class PkwareInputStream extends InputStream {
     ...
     *             On IO failure
     */
    public PkwareInputStream(InputStream input, int length) throws IOException {
        this.input = input;
    ...
    private void readHeader() throws IOException {
        ...
        int c = input.read();
        if (c != 0) {
            throwError("Static dictionary not supported");
            return;
        }
        ...
        switch (dictionaryBits) {
            ...
            case 6:
                ...
                break;
            default:
                throwError("Unknown dictionary size");
                return;
        }
     ...
     */
    private int getCopyLength() throws IOException {
        int bits;
     ...
     */
    private int getCopyOffset(int length) throws IOException {
     ...
     */
    private int getCopyOffsetHigh() throws IOException {
        int bits = readBits(2);
        if (bits == 3) { // 11
            return 0;
        } else if (bits == 1) { // 10
     ...
     */
    private void advanceByte() throws IOException {
        bufOffset++;
